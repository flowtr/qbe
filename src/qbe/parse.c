...
static void expect(int t) {
  static char *ttoa[] = {
      [Tlbl] = "label",  [Tcomma] = ",",  [Teq] = "=",
      [Tnl] = "newline", [Tlparen] = "(", [Trparen] = ")",
      [Tlbrace] = "{",   [Trbrace] = "}", [Teof] = 0,
  };
  char buf[128], *s1, *s2;
  int t1;
...
static PState parseline(PState ps) {
  ...
  case Tlbl:
    ...
    *blink = b;
    ...
    return PPhi;
  case Tret:
    curb->jmp.type = (int[]){Jretw, Jretl, Jrets, Jretd, Jretc, Jret0}[rcls];
    if (peek() == Tnl)
      curb->jmp.type = Jret0;
    else if (rcls < 5) {
      r = parseref();
...
static void parsedat(void cb(Dat *, FILE *), Lnk *lnk, FILE *outf) {
  char name[NString] = {0};
  int t;
  Dat d;
...
void parse(FILE *f, char *path, FILE *outf, void data(Dat *, FILE *),
           void func(Fn *, FILE *)) {
  Lnk lnk;
  uint n;
