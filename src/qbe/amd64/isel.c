...
static void fixarg(Ref *r, int k, Ins *i, Fn *fn) {
  char buf[32];
  Addr a, *m;
  Ref r0, r1;
  int s, n, op;
  ...
  } else if (!(isstore(op) && r == &i->arg[1]) && !isload(op) && op != Ocall &&
...
static void seladdr(Ref *r, ANum *an, Fn *fn) {
  Addr a;
  Ref r0;
...
static int cmpswap(Ref arg[2], int op) {
  switch (op) {
  case NCmpI + Cflt:
  case NCmpI + Cfle:
    return 1;
...
static void selcmp(Ref arg[2], int k, int swap, Fn *fn) {
  Ref r;
  Ins *icmp;
...
static void sel(Ins i, ANum *an, Fn *fn) {
  Ref r0, r1, tmp[7];
  ...
  case Ourem:
    ...
    if (fn->tmp[r0.val].slot != -1)
      err("unlikely argument %%%s in %s", fn->tmp[r0.val].name,
          optab[i.op].name);
...
static void seljmp(Blk *b, Fn *fn) {
  Ref r;
  int c, k, swap;
  Ins *fi;
  Tmp *t;
  ...
  if (!fi || !req(fi->to, r)) {
    ...
    b->jmp.type = Jjf + Cine;
  } else if (iscmp(fi->op, &k, &c) && c != NCmpI + Cfeq /* see sel() */
             && c != NCmpI + Cfne) {
    ...
    b->jmp.type = Jjf + c;
             ...
             (rtype(fi->arg[0]) == RTmp || rtype(fi->arg[1]) == RTmp)) {
    fi->op = Oxtest;
    fi->to = R;
    b->jmp.type = Jjf + Cine;
...
static int ascale(Ref r, Con *con) {
  int64_t n;
...
static void anumber(ANum *ai, Blk *b, Con *con) {
  ...
  static char add[10][10] = {
      [2][4] = 4, [4][2] = 4, [2][6] = 6, [6][2] = 6,
      [2][7] = 7, [7][2] = 7, [0][2] = 4, [2][0] = 4, /* 4: o + b */
      [0][0] = 5,                                     /* 5: b + s * i */
      [0][3] = 5, [3][0] = 5, [2][3] = 6, [3][2] = 6, /* 6: o + s * i */
      [2][5] = 7, [5][2] = 7,                         /* 7: o + b + s * i */
      [0][6] = 7, [6][0] = 7, [4][3] = 7, [3][4] = 7,
  };
...
static int amatch(Addr *a, Ref r, int n, ANum *ai, Fn *fn) {
  Ins *i;
  int nl, nr, t, s;
  Ref al, ar;
  ...
  if (rtype(r) == RCon) {
    if (!addcon(&a->offset, &fn->con[r.val]))
      err("unlikely sum of $%s and $%s", str(a->offset.label),
          str(fn->con[r.val].label));
    return 1;
  }
  assert(rtype(r) == RTmp);
  i = ai[r.val].i;
