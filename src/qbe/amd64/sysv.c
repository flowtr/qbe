...
static void classify(AClass *a, Typ *t, uint s) {
  Field *f;
  int *cls;
  uint n, s1;
...
static void typclass(AClass *a, Typ *t) {
  uint sz, al;
...
static int retr(Ref reg[2], AClass *aret) {
...
static int argsclass(Ins *i0, Ins *i1, AClass *ac, int op, AClass *aret,
                     Ref *env) {
...
int amd64_sysv_rsave[] = {RDI,   RSI,   RDX,   RCX,  R8,   R9,    R10,
                          R11,   RAX,   XMM0,  XMM1, XMM2, XMM3,  XMM4,
...
bits amd64_sysv_retregs(Ref r, int p[2]) {
  bits b;
  int ni, nf;
...
bits amd64_sysv_argregs(Ref r, int p[2]) {
  bits b;
  int j, ni, nf, ra;
...
static Ref rarg(int ty, int *ni, int *ns) {
  if (KBASE(ty) == 0)
...
static void selcall(Fn *fn, Ins *i0, Ins *i1, RAlloc **rap) {
  Ins *i;
  AClass *ac, *a, aret;
  int ca, ni, ns, al;
  uint stk, off;
...
static int selpar(Fn *fn, Ins *i0, Ins *i1) {
  AClass *ac, *a, aret;
  Ins *i;
  int ni, ns, s, al, fa;
  Ref r, env;
...
static void selvastart(Fn *fn, int fa, Ref ap) {
  Ref r0, r1;
  int gp, fp, sp;
