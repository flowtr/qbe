...
static char *rname[] = {
    ...
    "fs5",         "fs6",         "fs7",
    "fs8",         "fs9",         "fs10",
    "fs11",        [T6] = "t6",   [FT11] = "ft11",
};

static int64_t slot(int s, Fn *fn) {
  s = ((int32_t)s << 3) >> 3;
  assert(s <= fn->slot);
  if (s < 0)
    return 8 * -s;
...
static void emitaddr(Con *c, FILE *f) {
  char off[32], *p;
  ...
  for (;;) {
    ...
    case 'M':
      ...
      case RCon:
        ...
        emitaddr(pc, f);
        if (isstore(i->op) || (isload(i->op) && KBASE(i->cls) == 1)) {
...
static void fixslot(Ref *pr, Fn *fn, FILE *f) {
  Ref r;
  int64_t s;
