...
bits rv64_retregs(Ref r, int p[2]) {
  bits b;
  int ngp, nfp;
...
bits rv64_argregs(Ref r, int p[2]) {
  bits b;
  int ngp, nfp, t5;
...
static int fpstruct(Typ *t, int off, Class *c) {
  Field *f;
  int n;
...
static void typclass(Class *c, Typ *t, int fpabi, int *gp, int *fp) {
  uint n;
  int i;
...
static void sttmps(Ref tmp[], int ntmp, Class *c, Ref mem, Fn *fn) {
  static int st[] = {
...
static void ldregs(Class *c, Ref mem, Fn *fn) {
  int i;
  Ref r;
...
static int argsclass(Ins *i0, Ins *i1, Class *carg, int retptr) {
...
static void stkblob(Ref r, Typ *t, Fn *fn, Insl **ilp) {
  Insl *il;
  int al;
  uint64_t sz;
...
static void selvaarg(Fn *fn, Ins *i) {
  Ref loc, newloc;
