...
Ref newcon(Con *c0, Fn *fn) {
  ...
  for (i = 0; i < fn->ncon; i++) {
    c1 = &fn->con[i];
    if (c0->type == c1->type && c0->bits.i == c1->bits.i &&
        c0->label == c1->label && c0->local == c1->local)
      return CON(i);
  }
...
void blit(Ref rdst, uint doff, Ref rsrc, uint boff, uint sz, Fn *fn) {
  struct {
    int st, ld, cls, size;
...
void blit0(Ref rdst, Ref rsrc, uint sz, Fn *fn) {
...
void salloc(Ref rt, Ref rs, Fn *fn) {
  ...
  if (rtype(rs) == RCon) {
    ...
    if (fn->tmp[rs.val].slot != -1)
      err("unlikely alloc argument %%%s for %%%s", fn->tmp[rs.val].name,
          fn->tmp[rt.val].name);
  }
}
