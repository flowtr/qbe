...
#define X(c, str) {Oflag + c, Ki, "cset %=, " str},
            CMP(X)
#undef X
                {NOp, 0, 0}};
...
static char *rname(int r, int k) {
  ...
  if (r == SP) {
    ...
    default:
      die("invalid class");
    case Kw:
      sprintf(buf, "w%d", r - R0);
      break;
    case Kx:
    case Kl:
      sprintf(buf, "x%d", r - R0);
      break;
    }
  else if (V0 <= r && r <= V30)
    switch (k) {
    default:
      die("invalid class");
    case Ks:
      sprintf(buf, "s%d", r - V0);
      break;
    case Kx:
    case Kd:
      sprintf(buf, "d%d", r - V0);
      break;
    }
  else
...
static uint64_t slot(int s, E *e) {
  s = ((int32_t)s << 3) >> 3;
  if (s == -1)
    return 16 + e->frame;
  if (s < 0) {
...
static void loadcon(Con *c, int r, int k, FILE *f) {
  char *rn, *p, off[32];
  int64_t n;
  int w, sh;
...
static void fixarg(Ref *pr, int sz, E *e) {
  Ins *i;
  Ref r;
  uint64_t s;
...
void arm64_emitfn(Fn *fn, FILE *out) {
  static char *ctoa[] = {
#define X(c, s) [c] = s,
      CMP(X)
#undef X
  ...
  if (e->frame + 16 <= 512)
    fprintf(e->f, "\tstp\tx29, x30, [sp, -%" PRIu64 "]!\n", e->frame + 16);
  ...
  for (lbl = 0, b = e->fn->start; b; b = b->link) {
    ...
    case Jret0:
      ...
      if (e->fn->vararg)
        o += 192;
      if (o <= 504)
        fprintf(e->f, "\tldp\tx29, x30, [sp], %" PRIu64 "\n", o);
      else if (o - 16 <= 4095)
        fprintf(e->f,
                "\tldp\tx29, x30, [sp], 16\n"
                "\tadd\tsp, sp, #%" PRIu64 "\n",
