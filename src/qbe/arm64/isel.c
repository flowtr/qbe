...
enum Imm { Iother, Iplo12, Iphi12, Iplo24, Inlo12, Inhi12, Inlo24 };

static enum Imm imm(Con *c, int k, int64_t *pn) {
  int64_t n;
  int i;
...
int arm64_logimm(uint64_t x, int k) {
  uint64_t n;
...
static void fixarg(Ref *pr, int k, int phi, Fn *fn) {
  char buf[32];
  Ref r0, r1, r2;
  int s, n;
  Con *c;
...
static int selcmp(Ref arg[2], int k, Fn *fn) {
  Ref r, *iarg;
  Con *c;
  int swap, cmp, fix;
  int64_t n;
