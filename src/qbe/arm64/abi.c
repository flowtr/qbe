...
static int isfloatv(Typ *t, char *cls) {
  Field *f;
  uint n;
...
static void typclass(Class *c, Typ *t, int *gp, int *fp) {
  uint64_t sz;
  uint n;
...
static void sttmps(Ref tmp[], int cls[], uint nreg, Ref mem, Fn *fn) {
  static int st[] = {
      [Kw] = Ostorew, [Kl] = Ostorel, [Ks] = Ostores, [Kd] = Ostored};
  uint n;
  uint64_t off;
  Ref r;
...
static void ldregs(int reg[], int cls[], int n, Ref mem, Fn *fn) {
  int i;
  uint64_t off;
  Ref r;
...
static int argsclass(Ins *i0, Ins *i1, Class *carg) {
...
bits arm64_retregs(Ref r, int p[2]) {
  bits b;
  int ngp, nfp;
...
bits arm64_argregs(Ref r, int p[2]) {
  bits b;
  int ngp, nfp, x8, x9;
...
static void stkblob(Ref r, Class *c, Fn *fn, Insl **ilp) {
  Insl *il;
  int al;
  uint64_t sz;
...
static void selcall(Fn *fn, Ins *i0, Ins *i1, Insl **ilp) {
  Ins *i;
  Class *ca, *c, cr;
  int cty;
  uint n;
...
static Params selpar(Fn *fn, Ins *i0, Ins *i1) {
  Class *ca, *c, cr;
  Insl *il;
  Ins *i;
  int n, s, cty;
  ...
  return (Params){
      .nstk = s - 2, .ngp = (cty >> 5) & 15, .nfp = (cty >> 9) & 15};
}

static Blk *split(Fn *fn, Blk *b) {
  Blk *bn;
...
static void chpred(Blk *b, Blk *bp, Blk *bp1) {
  Phi *p;
  uint a;
...
static void selvaarg(Fn *fn, Blk *b, Ins *i) {
...
static void selvastart(Fn *fn, Params p, Ref ap) {
  Ref r0, r1, rsave;
